...
const get_Filter_Doctor_List = async (specialty, region) => {
    try {
        ...
        if (error.response) {
            console.log("Error doctor by specialty and region: ", error.response.data.error)
            return error.response.data.error
        } else {
...
const addDoctorActiveHour = async (doctor_id, activeHour) => {
    try {
        const response = await client.post(`/doc/add-active-hour/${doctor_id}`, activeHour)
        return response.data
    } catch (error) {
        if (error.response) {
...
const updateDoctorActiveHour = async (doctor_id, newActiveHour, oldActiveHour) => {
    try {
        ...
        const response = await client.post(`/doc/update-active-hour/${doctor_id}`, payload)
...
const deleteDoctorActiveHour = async (doctor_id, activeHour) => {
    try {
        ...
        const response = await client.post(`/doc/delete-active-hour/${doctor_id}`, activeHour)

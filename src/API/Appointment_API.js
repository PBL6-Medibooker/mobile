...
import { parseAppointmentEndDate, parseAppointmentStartDate } from "../utils/ConvertDate"
    ...
    try {
        ...
        if (appointments && Array.isArray(appointments) && user_id) {
            ...
            const upcoming = [...appointments]
                .filter((item) => item.user_id._id === user_id && item.is_deleted === false)
                .sort(sortAppointments)

            const complete = [...appointments]
                .filter((item) => item.user_id._id === user_id && item.is_deleted === true)
                .sort(sortAppointments)
...
const soft_delete_Appointment = async (appointmentId) => {
    try {
        const response = await client.post(`/appointment/soft-delete-appointment/${appointmentId}`)
        return response.data
    } catch (error) {
        if (error.response) console.log("Error delete appointment: ", error.response.data.error)
...
const restore_Appointment = async (appointmentId) => {
    try {
        const response = await client.post(`/appointment/restore-appointment/${appointmentId}`)
        return response.data
    } catch (error) {
        if (error.response) console.log("Error restore appointment: ", error.response.data.error)
...
const canncel_Appointment = async (appointmentId) => {
    try {
        const response = await client.post(`/appointment/cancel-appointment/${appointmentId}`)
        // console.log(response.data);

        return response.data
    } catch (error) {
        if (error.response) console.log("Error cancel appointment: ", error.response.data.error)
...
const get_Appointment_Of_Doctor = async (doctor_id) => {
    try {
        const response = await client.post(`/appointment/get-appointment-by-doctor/${doctor_id}`)
        const appointments = response.data
        ...
        if (error.response) {
            console.log("Error get appointment of doctor: ", error.response.data.error)
        } else {
...
const get_Doctor_Appointment_By_Status = async (doctor_id) => {
    try {
        ...
        if (appointments && Array.isArray(appointments) && doctor_id) {
            ...
            const upcoming = [...appointments]
                .filter((item) => item?.doctor_id?._id === doctor_id && item?.is_deleted === false)
                .sort(sortAppointments)

            const complete = [...appointments]
                .filter((item) => item.doctor_id._id === doctor_id && item.is_deleted === true)
                .sort(sortAppointments)
